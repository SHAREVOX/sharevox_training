...
from models.vits.transform import piecewise_rational_quadratic_transform


class FlipFlow(nn.Module):
    """Flip flow module."""

    def forward(self, x: Tensor, *args, inverse: bool = False, **kwargs) -> Union[Tensor, Tuple[Tensor, Tensor]]:
...
class ElementwiseAffineFlow(nn.Module):
    ...
    def __init__(self, channels: int):
        ...
        self.register_parameter("m", nn.Parameter(torch.zeros(channels, 1)))
        self.register_parameter("logs", nn.Parameter(torch.zeros(channels, 1)))

    def forward(
        self, x: Tensor, x_mask: Tensor, inverse: bool = False, **kwargs
    ) -> Union[Tensor, Tuple[Tensor, Tensor]]:
        """Calculate forward propagation.
        Args:
            x (Tensor): Input tensor (B, channels, T).
...
class Transpose(nn.Module):
    """Transpose module for nn.Sequential()."""
