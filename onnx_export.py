...
class Variance(nn.Module):
    ...
    def forward(
        ...
        log_pitches, log_durations = self.variance_model(phonemes, accents, speakers)
        durations = torch.clamp((torch.exp(log_durations) - 1) / (self.sampling_rate / self.hop_length), min=0.01)
        return log_pitches, durations
...
class Decoder(nn.Module):
    ...
    def forward(self, length_regulated_tensor: Tensor) -> Tensor:
        ...
        wavs = self.fregan(postnet_outputs[0].transpose(0, 1).unsqueeze(0)).squeeze(1) * self.max_wav_value
        wavs = wavs.to(torch.int16)
        return wavs
